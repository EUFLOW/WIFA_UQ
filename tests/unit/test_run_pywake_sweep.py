...
class TestSetNestedDictValue:
    ...
    def test_windio_style_path(self):
        ...
        assert d["attributes"]["analysis"]["wind_deficit_model"]["wake_expansion_coefficient"]["k_b"] == 0.07
...
class TestRunParameterSweep:
    ...
    def test_output_shapes_and_metadata(self, mock_run_pywake, tmp_path, windio_system_dict):
    ...
    @patch("wifa_uq.model_error_database.run_pywake_sweep.run_pywake")
    def test_bias_calculation_correctness(self, mock_run_pywake, tmp_path, windio_system_dict):
        ...
        np.testing.assert_allclose(merged.model_bias_cap.values, expected_bias, rtol=1e-10)

...
class PreprocessingInputs:
    def __init__(self, ref_resource_path: Path, output_path: Path, steps: list[str] = None):
        """
        ...
        if not self.ref_resource_path.exists():
            raise FileNotFoundError(f"Input resource file not found: {self.ref_resource_path}")
    ...
    def run_pipeline(self) -> Path:
        ...
        """
        if not self.steps:
            print(f"No steps to run. Copying {self.ref_resource_path.name} to {self.output_path.name}")
        ...
        if 'update_heights' in self.steps:
            print("  WARNING: Skipping 'update_heights'. This step is not fully supported "
        ...
        try:
            with xr.load_dataset(input_path) as ref_inputs:
                ...
                if "k" in ref_inputs and "wind_speed" in ref_inputs:
                    ...
                    print("    Skipping TI recalculation: 'k' or 'wind_speed' not found.")
                
                # --- Calculate Wind Veer ---
                try:
                    ref_inputs_modified["wind_veer"] = _calculate_veer(ref_inputs_modified)
                except Exception as e:
                    print(f"    Skipping wind veer calculation: {e}")

                # --- Try for velocity based ABL ---
                if "wind_speed" in ref_inputs_modified and "height" in ref_inputs_modified:
                    try:
                        ref_inputs_modified["ABL_height"] = _calculate_abl_from_velocity(ref_inputs_modified)
                    except Exception as e:
                ...
                if ("height" in ref_inputs_modified and "potential_temperature" in ref_inputs_modified):
                    
                    non_height_dims = [d for d in ref_inputs_modified["potential_temperature"].dims if d != "height"]
                    non_height_shape = [s for d, s in ref_inputs_modified["potential_temperature"].sizes.items() if d != "height"]

                    if "LMO" not in ref_inputs_modified:
                        print("    'LMO' not found. Assuming neutral stability (LMO=1e10).")
                        ref_inputs_modified["LMO"] = xr.DataArray(
                            np.full(non_height_shape, 1e10),
                            dims=non_height_dims
                        )
                    
                    if "ABL_height" in ref_inputs_modified:
                        abl_guess = ref_inputs_modified["ABL_height"]
                    else:
                        print("    'ABL_height' (initial guess) not found. Assuming 1000m.")
                        abl_guess = xr.DataArray(
                            np.full(non_height_shape, 1000.0),
                            dims=non_height_dims
                        )

                    print("    Running 'ci_fitting' for thermal parameters (lapse_rate, etc.)...")
                    H_temp, dthdz, dth, inv_thickness, unknown1, unknown2 = xr.apply_ufunc(
                        ...
                        vectorize=True,
                        dask="allowed",
                        output_dtypes=[float, float, float, float, float, float], # <-- UPDATED
                    )
                    ...
                    print("    Skipping temperature-based calculations: 'height' or 'potential_temperature' not found.")
            ...
            print(f"    FATAL Error during 'recalculate_params': {type(e).__name__}: {e}")

...
nc_path = os.path.join(
    os.path.dirname(__file__), "results", "rmse", "calibration_results.nc"
)
output_dir = (
    Path(os.path.dirname(__file__)) / "results" / "rmse" / "figures" / "gaussproc_reg"
)
...
with xr.open_dataset(nc_path) as ds:
    ...
    def read_data(name):
        if name == "farmPowerNormalized":
            return (
                ds["best_k_farm_P"].values.flatten() / ds["capacity"].values.flatten()
            )
        elif name == "ABLHH":
            ...
            return (
                ds["capping_inversion_thickness"].values.flatten()
                / ds["ABL_height"].values.flatten()
            )
        else:
    ...
    for target in targets:
        ...
        if rank < X_.shape[1]:
            if not allow_svd:
                raise ValueError(
                    f"Feature matrix is rank deficient (rank={rank}, features={X_.shape[1]}). Set allow_svd=True to reduce rank using SVD."
                )
            print(
                f"Feature matrix is rank deficient (rank={rank}, features={X_.shape[1]}). Reducing rank using SVD."
            )
            # SVD decomposition
        ...
        kernel = C(1.0, (1e-3, 1e3)) * Matern(length_scale=1.0, nu=nu) + WhiteKernel(
            noise_level=1e-7
        )
        model = GaussianProcessRegressor(
            kernel=kernel, n_restarts_optimizer=10, alpha=0, normalize_y=True
        )
        ...
        if hasattr(model.kernel_, "k2") and hasattr(model.kernel_.k2, "length_scale"):
            ...
            if X_.shape[1] == len(feature_vars):
                ...
                plt.barh(
                    [f"SVD_{i + 1}" for i in range(X_.shape[1])],
                    model.kernel_.k2.length_scale,
                )
        ...
        if X_.shape[1] == len(feature_vars):
            ...
            for i, var in enumerate(feature_vars):
                ...
                var_range = np.linspace(
                    np.percentile(X_[:, i], 1), np.percentile(X_[:, i], 99), 100
                )
            ...
            print(
                "Feature matrix was rank reduced; skipping feature-specific dependency plots."
            )
        ...
        for i, j in combinations(range(num_axes), 2):
            ...
            if var2 == "k":
                var2 = "RANS_k"
            v1_range = np.linspace(
                np.percentile(X_[:, i], 1), np.percentile(X_[:, i], 99), 40
            )
            v2_range = np.linspace(
                np.percentile(X_[:, j], 1), np.percentile(X_[:, j], 99), 40
            )
            ...
            if num_axes == len(feature_vars):
                ax.set_title(
                    f"Dependence of {target} on {var1} and {var2}\n(others fixed at median)"
                )
            else:
                ax.set_title(
                    f"Dependence of {target} on {var1} and {var2} (SVD components)"
                )
            ...
            fig = plt.figure(figsize=(8, 6))
            ax = fig.add_subplot(111, projection="3d")
            rel_uncertainty = np.divide(
                Y_std, np.abs(Y_pred) + 1e-8
            ...
            if num_axes == len(feature_vars):
                ax.set_title(
                    f"Relative Uncertainty of {target} on {var1} and {var2}\n(others fixed at median)"
                )
            else:
                ax.set_title(
                    f"Relative Uncertainty of {target} on {var1} and {var2} (SVD components)"
                )
            ...
            fig_pred = go.Figure(
                data=[go.Surface(z=Y_pred, x=V1, y=V2, colorscale="Viridis")]
            )
            fig_pred.update_layout(
                ...
                scene=dict(
                    xaxis_title=var1,
                    yaxis_title=var2,
                    zaxis_title=f"Predicted {target}",
                ),
            )
            ...
            fig_unc = go.Figure(
                data=[go.Surface(z=rel_uncertainty, x=V1, y=V2, colorscale="Plasma")]
            )
            fig_unc.update_layout(
                ...
                scene=dict(
                    xaxis_title=var1,
                    yaxis_title=var2,
                    zaxis_title="Relative Uncertainty",
                ),
            )
        ...
        if len(plotly_figs) > 0:
            ...
            for idx in range(0, len(plotly_figs), 2):
                ...
                unc_title, unc_fig = (
                    plotly_figs[idx + 1] if idx + 1 < len(plotly_figs) else (None, None)
                )
                row = pn.Row(
                    pn.pane.Plotly(
                        pred_fig,

...
def plot_training_quality(
    ...
    if plot_options.get("scatter", False):
        ...
        for idx, it in enumerate(time_indices):
            ...
            for var_name in stochastic_varnames_physical:
                value_str = (
                    f"{var_name}={np.array(calibration_inputs[var_name])[it]:.4g}"
                )
            ...
            ax[idx].grid(True)
            sc1 = ax[idx].plot(
                model_bias_cap[:, it], PCE_realizations[:, it], "r.", label="PCE"
            )
            sc2 = ax[idx].plot(
                model_bias_cap[:, it], model_bias_cap[:, it], "k--", label="y=x"
            )
            if idx == 0:
    ...
    if plot_options.get("distribution", False):
        fig, ax = plt.subplots(1, 4, figsize=(18, 6))
        xs = np.linspace(
            min(model_bias_cap.min(), PCE_realizations.min()),
            max(model_bias_cap.max(), PCE_realizations.max()),
            1000,
        )
...
def run_pce_sensitivity(X, y, feature_names, pce_config: dict, output_dir: Path):
    ...
    if len(feature_names) != n_features:
        raise ValueError(
            f"X has {n_features} features but {len(feature_names)} feature names provided"
        )
    ...
    for _, row in sobol_df.iterrows():
        print(
            f"        {row['Feature']}: S1={row['First_Order']:.4f}, ST={row['Total_Order']:.4f}"
        )

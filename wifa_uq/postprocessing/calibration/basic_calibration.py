        ...
        for param_name in self.swept_params:
            if param_name in self.dataset_train.coords:
                self.best_params_[param_name] = float(
                    self.dataset_train.coords[param_name]
                    .isel(sample=self.best_idx_)
                    .values
                )
...
class LocalParameterPredictor:
    ...
    def fit(self):
        ...
        for case_idx in range(n_cases):
            ...
            bias_values = (
                self.dataset_train["model_bias_cap"].isel(case_index=case_idx).values
            )

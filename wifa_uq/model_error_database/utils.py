...
def calc_boundary_area(x, y, show=True): 
    ...
    if show:
        ...
        # Close the hull
        plt.plot([points[hull.vertices[0], 0], points[hull.vertices[-1], 0]],
                 [points[hull.vertices[0], 1], points[hull.vertices[-1], 1]], "r-")
...
def blockage_metrics(xy, wind_dir_deg_from_north, D, grid_res=151, L_inf_factor=20.0,plot=False):
    """
    ...
    for i in range(N):
        ...
        for j in range(cross_dist.size):
            ...
            hits = d <= R                   # boolean array of rotor grid cells that are blocked by this upstream turbine
    ...
    if plot:
        def plot_metric(metric, title, cmap='viridis'):
            ...
            sc = ax.scatter(xy[:,0], xy[:,1], c=metric, s=100, cmap=cmap, edgecolor='k')
            ...
            ax.arrow(np.mean(xy[:,0]), np.mean(xy[:,1]),
                     -arrow_length*np.sin(theta), -arrow_length*np.cos(theta),
        ...
        plot_metric(BR, f'Blockage Ratio per Turbine: Farm Average = {BR_farm:.2f}', cmap='viridis',)

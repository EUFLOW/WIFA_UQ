...
wind_diffs = []
np.random.seed(3)
kk_samples = np.random.uniform(0.01, 0.3, 30)
for ii, kk in enumerate(kk_samples):
    dat['attributes']['analysis']['wind_deficit_model']['wake_expansion_coefficient']['k_b'] = kk
    run_pywake(dat, output_dir='sample_%i' % ii)

    flow_field = xr.load_dataset(f'sample_{ii}/FarmFlow.nc')
    ...
    plt.title('k=%.2f' % kk)
    plt.savefig('figs/sample_%i.png' % ii)
    plt.clf()
...
final_diffs = xr.concat(wind_diffs, dim=pd.Index(kk_samples, name='kk'))
best_ks = kk_samples[final_diffs.argmin('kk').values]
...
for tt in range(finaldat.time.size):
    ...
    flow_field = xr.load_dataset(f'sample_%i/FarmFlow.nc' % np.where(np.isclose(kk_samples, this_k))[0][0]).isel(z=0, time=tt)
...
model = LinearRegression()
model.fit(np.atleast_2d(X[X.columns[-1]]).T, Y)
tis = np.linspace(0, .4,)
preds = model.predict(np.atleast_2d(tis).T)
plt.title('k = %.3f TI + %.3f' % (model.coef_[0], model.intercept_))
plt.scatter(X[X.columns[-1]], Y)
plt.plot(tis, preds, c='k', ls='--')
plt.xlabel('TI')
plt.ylabel('k')
plt.savefig('TI_K')
plt.clf()
